...
def create_drift(model, token):
    ...
    response = requests.post(
        url=f"https://{settings.DRIFT_MONITOR_DOMAIN}/api/latest/drift",
...
def complete_drift(drift, token):
    ...
    response = requests.put(
        url=f"https://{settings.DRIFT_MONITOR_DOMAIN}/api/latest/drift/{drift['id']}",
        headers={"Authorization": f"Bearer {token}"},
...
def register(token):
    ...
    response = requests.post(
        url=f"https://{settings.DRIFT_MONITOR_DOMAIN}/api/latest/user",
        headers={"Authorization": f"Bearer {token}"},
        timeout=settings.DRIFT_MONITOR_TIMEOUT,
        verify=not settings.TESTING,
    )
